...
- every visible interactive element (button, link, tappable row) has a
  bounding box of at least 44x44px — enforces the CLAUDE.md tap-target
  rule (Apple HIG minimum). The pre-existing violations (username pill,
  delete X buttons, meeting detail chips, RSVP buttons) were fixed on
  2026-07-10 so the suite starts green.
